...
export default function AddQuestionJson({onSuccess,onError, setModalClose, id }) {
            ...
            <div className='mt-4 bg-gray-100 rounded-md p-4 max-h-[300px] overflow-y-auto'>
              ...
              <ul className='list-disc pl-5 space-y-2 text-sm'>
                {previewData.map((item, index) => (
                  <li key={index}>
                    <strong>Q{index + 1}:</strong> {item.question} &mdash; <i>{item.category}</i> ({item.marks} marks)
                  </li>
                ))}
              </ul>
            </div>

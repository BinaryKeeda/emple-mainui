...
import {
  ...
  BookOutlined,
  WorkOutlineOutlined,
  AccountBalanceOutlined, // ✅ CHANGED: added for Finance
  Close,
  ReceiptLong,
  ReceiptOutlined,        // ✅ CHANGED: added for Transactions
  SmartToyOutlined,
  ShoppingCartOutlined,
  MapOutlined,
  FolderOutlined,
...
export default function Sidebar({ showMenu, setShowMenu }: {
  ...
  const NAV_ITEMS = [
    ...
    // ✅ CHANGED: Replaced Upes with Finance
    {
    ...
    {
    icon: <WorkOutlineOutlined sx={{ fontSize: 24 }} />,
    label: 'Jobs',
    path: '/user/jobs',
    type: 'public',
    badge: null  
  ...
  return (
    <>
      ...
      >
        ...
        {/* ✅ CHANGED: Transactions pinned at bottom */}
        <div style={{ padding: '0.75rem', borderTop: '1px solid #e5e7eb' }}>
          {(() => {
            const transPath = '/user/transactions'
            const isActive = location.pathname === transPath
            const isHovered = hoveredItem === 999
            return (
              <div
                ...
                style={{
                  ...
                  background: isActive
                    ? 'linear-gradient(135deg, #ff6200 0%, #f13000 100%)'
                    : isHovered ? 'rgba(255, 107, 53, 0.08)' : 'transparent',
                  color: isActive ? 'white' : isHovered ? '#ff6b35' : '#000000',
                  fontWeight: isActive ? '600' : '500',
                  fontSize: '0.875rem',
              ...
              >
                <Link
                  to={transPath}
                  style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', width: '100%', textDecoration: 'none', color: 'inherit' }}
                >
                  {isActive && (
                    <div style={{
                      position: 'absolute',
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', transition: 'transform 0.3s ease', transform: isHovered ? 'scale(1.1)' : 'scale(1)', color: 'inherit' }}>
                    <ReceiptOutlined sx={{ fontSize: 24 }} />
                  </span>
        ...
        {showMenu && (
          ...
          >
            ...
            >
              ...
              {/* ✅ CHANGED: Transactions in mobile menu too */}
              <Link
              ...
              >
                <span className='mt-1 text-sm text-white text-center'>Transactions</span>
              </Link>
              {data?.data?.map((item: any) => (
                <Link
                  key={item._id}
